...
export async function generateMetadata({ params }: Props) {
  ...
  for (const language of ["vi","en","zh"] as const) {
    if (language === p.locale && (r || s)) languages[language] = `/${language}/${p.section}/${p.slug}`;
    else if (r?.translationKey) {
      const translated = (await getRecords(p.section,language)).find(item=>item.translationKey===r.translationKey);
      if (translated) languages[language]=`/${language}/${p.section}/${translated.slug}`;
  ...
  return pageMetadata({
    locale: p.locale,
    title: r?.seo?.title || r?.title || (s ? t(p.locale, s.vi, s.en) : "404"),
    description:
      r?.seo?.description ||
      r?.summary ||
...
export default async function Detail({ params, searchParams }: Props) {
  ...
  // Bản xem trước chưa xuất bản không được phát dữ liệu có cấu trúc.
  const structured = preview ? [] : structuredDataFor(record, locale, section, path);
  return (
    <>
      ...
      <PageHeading
        ...
        title={title}
        summary={
          record?.summary || (sample ? t(locale,sample.description[0],sample.description[1]) : undefined)
        }
      />
      <div className="detail-back"><Link href={`/${locale}/${section}`}>← {t(locale,"Trở về danh sách","Back to listing")}</Link></div>
